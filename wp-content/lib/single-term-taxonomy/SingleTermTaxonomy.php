...
if (!class_exists('SingleTermTaxonomy')) :
  /**
  ...
  class SingleTermTaxonomy {
     ...
     */
    public function __construct($tax_slug, $post_types = [], $type = 'radio', $default = []) {

      $this->slug = $tax_slug;
      $this->post_types = is_array($post_types) ? $post_types : [$post_types];
      $this->input_element = in_array((string) $type, ['radio', 'select']) ? $type : $this->input_element;
      $this->default = $this->process_default($default);

      add_action('add_meta_boxes', [$this, 'add_input_element']);
      add_action('admin_footer', [$this, 'js_checkbox_transform']);
      add_action('wp_ajax_taxonomy_single_term_add', [$this, 'ajax_add_term']);
    ...
    protected function process_default($default = []) {
      ...
      if (empty($default)) {
        $default = [(int) get_option('default_' . $this->slug)];
      }
    ...
    public function add_input_element() {
      ...
      foreach ($this->post_types() as $key => $cpt) {
        ...
        add_meta_box($this->slug . '_input_element', $this->metabox_title(), [$this, 'input_element'], $cpt, $this->context, $this->priority);
      }
    }
  ...
  public function select_open() {
    ?>
    <select style="display:block;width:100%;margin-top:12px;" name="<?php
    echo $this->namefield; ?>" id="<?php
    echo $this->slug; ?>checklist" class="form-no-clear">
      <?php
      if (!$this->force_selection) : ?>
        <option value="0"><?php
          echo esc_html(apply_filters('taxonomy_single_term_select_none', __('None'))); ?></option>
      <?php
      endif;
      }
      ...
      ?>
      <ul id="<?php
      echo $this->slug; ?>checklist" data-wp-lists="list:<?php
      echo $this->slug; ?>" class="categorychecklist form-no-clear">
        <?php
        if (!$this->force_selection) : ?>
          <li style="display:none;">
            <input id="taxonomy-<?php
            echo $this->slug; ?>-clear" type="radio" name="<?php
            echo $this->namefield; ?>" value="0"/>
          </li>
        <?php
        endif;
        }
    ...
    public function radio_close() {
      ...
      <p style="margin-bottom:0;float:left;width:50%;">
			<a class="button" id="taxonomy-<?php
      echo $this->slug; ?>-trigger-clear" href="#"><?php
        _e('Clear'); ?></a>
		</p>
     ...
     */
    public function term_fields_list($post_id = null, $args = []) {
    ...
    public function terms_adder_button() {
      ...
      <p style="margin-bottom:0;float:right;width:50%;text-align:right;">
			<a class="button-secondary" id="taxonomy-<?php
      echo $this->slug; ?>-new" href="#"<?php
      if ('radio' == $this->input_element) : ?> style="display:inline-block;margin-top:0.4em;"<?php
      endif; ?>><?php
        _e('Add New'); ?></a>
		</p>
      <script type="text/javascript">
    ...
    public function bulk_edit_handler() {
      ...
      } else if (isset($_REQUEST['tax_input'][$name]) && is_array($_REQUEST['tax_input'][$name])) {
      ...
      if ($this->to_set) {
        ...
        add_action('set_object_terms', [$this, 'maybe_resave_terms'], 10, 5);
      }
    }
    ...
    /**
     * Gets the SingleTermTaxonomyWalker object for use in term_fields_list and ajax_add_term
     *
     * @return object SingleTermTaxonomyWalker object
     */
    public function walker() {
      if ($this->walker) {
        return $this->walker;
      }
      require_once('SingleTermTaxonomyWalker.php');
     ...
     *
     * @return SingleTermTaxonomy  Returns Taxonomy_Single_Term object, allows for chaining.
     */
